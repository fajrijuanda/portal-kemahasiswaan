            ...
            @elseif($field['type'] === 'select')
                <select id="{{ $fieldId }}" name="{{ $name }}" class="form-select ubp-control @error($name) is-invalid @enderror" @required($field['required'] ?? false)>
                    @foreach($field['options'] as $option)
                        <option value="{{ $option }}" @selected((string) old($name, $record?->{$name}) === (string) $option)>{{ $option }}</option>
                    @endforeach
                </select>
            @elseif($field['type'] === 'file')
                <input id="{{ $fieldId }}" type="file" name="{{ $name }}" class="form-control ubp-control @error($name) is-invalid @enderror" accept="image/*">
                @if($record?->{$name})
